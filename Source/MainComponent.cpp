...
void MainComponent::configurePanels()
{
    ...
    runtimeLabel.setText(juce::String(creation_modeler::language::getLanguageRuntimeSummary()), juce::dontSendNotification);
    runtimeLabel.setColour(juce::Label::textColourId, creation_modeler::branding::accentColour());
...
{
    return creation::assets::SuiteAppDomain::modeler;
}
...
{
    juce::String text;
    text << "Project shell target: Creation Modeler\n";
    text << "Domain token: " << juce::String(creation_modeler::language::getAppDomainName()) << "\n\n";
...
{
    g.fillAll(creation_modeler::branding::backgroundColour());

    auto bounds = getLocalBounds().toFloat().reduced(18.0f);
    g.setColour(creation_modeler::branding::panelColour());
    g.fillRoundedRectangle(bounds, 24.0f);

    g.setColour(creation_modeler::branding::accentColour().withAlpha(0.8f));
